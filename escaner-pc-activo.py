...
def get_arguments():
	...
	parser.add_argument("-t", "--target", required=True, dest="target", help="Hosto o rango de red")
...
def parse_target(target_str):
	...
	if len(target_str_splitted) == 4:
		...
		print(colored(f"\n[!] El formato IP o rango de IP no es valido\n", 'red'))
...
def host_discovery(targets):
	...
	try:
		...
		if ping.returncode == 0:
			print(colored(f"\t[i] La IP {targets} esta activa", 'green'))
